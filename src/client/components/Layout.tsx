...
export default function Layout() {
      ...
      <div className="flex flex-col h-full">
        ...
        <div className="flex items-center gap-3 px-4 py-4 border-b border-slate-700/50 flex-shrink-0">
          ...
          {isExpanded && (
            <div className="overflow-hidden">
              <p className="font-bold text-sm text-white leading-tight truncate">물품관리</p>
              <p className="text-[10px] text-slate-400 leading-tight">호남 The Sum</p>
            </div>
          )}
          {isMobile && (
            <button onClick={() => setMobileOpen(false)} className="ml-auto p-1 text-slate-400 hover:text-white">
              <X className="w-5 h-5" />
            </button>
          )}
        </div>
        ...
        <div className="px-2 pt-3 pb-1">
          ...
          >
            ...
            {isExpanded && <span className="text-sm font-medium truncate">대시보드</span>}
          </NavLink>
        </div>
              ...
              <div key={section.key}>
                ...
                {isExpanded ? (
                  <button
                    ...
                    className="flex items-center justify-between w-full px-3 py-1.5 mt-2 mb-0.5"
                  >
                    <span className={`text-[10px] font-semibold uppercase tracking-wider ${hasActive ? 'text-teal-400' : 'text-slate-500'}`}>
                      {section.label}
                    </span>
                ...
                {!isCollapsed && section.items.map(item => {
                  ...
                  return (
                    <NavLink
                      ...
                      to={item.path}
                      className={({ isActive }) =>
                        `flex items-center gap-3 px-3 py-1.5 rounded-lg transition-all duration-150 relative group ${
                          isActive
                            ? 'bg-slate-700/60 text-white'
                    ...
                    >
                      {({ isActive }) => (
                        <>
                          {isActive && <div className="absolute left-0 top-1/2 -translate-y-1/2 w-[3px] h-5 bg-teal-400 rounded-r" />}
                          <Icon className="w-4.5 h-4.5 flex-shrink-0" style={{ width: 18, height: 18 }} />
                          {isExpanded && <span className="text-sm truncate">{item.label}</span>}
                          {!isExpanded && (
                            <div className="absolute left-full ml-2 px-2 py-1 bg-slate-800 text-white text-xs rounded shadow-lg opacity-0 group-hover:opacity-100 pointer-events-none whitespace-nowrap z-50 transition-opacity">
                              {item.label}
                            </div>
                          )}
                        </>
        ...
        {/* 하단 사용자 정보 */}
        <div className="flex-shrink-0 border-t border-slate-700/50 px-3 py-3">
          {isExpanded ? (
            <>
              ...
              <button
                ...
                className="flex items-center gap-2 text-xs text-red-400 hover:text-red-300 px-1 py-1 transition-colors"
              >
  ...
  return (
    <div className="min-h-screen bg-slate-50">
      {/* ── PC 사이드바 ──────────────────────────────────────────── */}
      <aside
        className={`hidden md:flex flex-col fixed top-0 left-0 bottom-0 bg-slate-900 z-40 transition-all duration-200 ${
          expanded ? 'w-56' : 'w-16'
        }`}
      ...
      {mobileOpen && (
        <div className="fixed inset-0 z-50 md:hidden">
          ...
          <div className="absolute top-0 left-0 bottom-0 w-64 bg-slate-900 shadow-xl overflow-hidden">
            {sidebarContent(true)}
          </div>
        </div>
      )}
