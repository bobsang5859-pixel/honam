...
export default function ExpenseScopesPage() {
  const { user } = useAuth();
  const canWrite = user?.permissions?.includes('BASIC_MANAGE');
  const [rows, setRows] = useState<ExpenseScope[]>([]);
  const [loading, setLoading] = useState(true);
  const [modal, setModal] = useState<'create' | 'edit' | null>(null);
  const [editing, setEditing] = useState<ExpenseScope | null>(null);
  ...
  const load = useCallback(() => {
    setLoading(true);
    api('/expense-scopes').then(setRows).catch(console.error).finally(() => setLoading(false));
  }, []);
  useEffect(() => { load(); }, [load]);
  ...
  const handleSubmit = async () => {
    ...
    try {
      if (modal === 'create') {
        await api('/expense-scopes', { method: 'POST', body: JSON.stringify(form) });
        showMsg('ok', '등록되었습니다.');
      } else if (editing) {
        await api(`/expense-scopes/${editing.id}`, { method: 'PUT', body: JSON.stringify(form) });
        showMsg('ok', '수정되었습니다.');
      }
      closeModal(); load();
    } catch (e: any) {
  ...
  const handleDelete = async (r: ExpenseScope) => {
    if (!confirm(`"${r.name}" 항목을 삭제하시겠습니까?`)) return;
    try {
      await api(`/expense-scopes/${r.id}`, { method: 'DELETE' });
  ...
  return (
    <div>
      <PageHeader
        icon={Wallet}
        title="비용구분 등록"
        description="비용 구분(expense_scope) 마스터 데이터를 관리합니다."
      ...
      {loading ? (
        ...
        <div className="card p-0 overflow-hidden overflow-x-auto">
          <table className="tbl">
            ...
            <tbody>
              {rows.length === 0 && (
                <tr><td colSpan={5} className="text-center py-10 text-slate-400">등록된 비용구분이 없습니다.</td></tr>
              )}
              {rows.map(r => (
                <tr key={r.id}>
      ...
      <Modal
        open={!!modal}
        onClose={closeModal}
        title={modal === 'create' ? '비용구분 추가' : '비용구분 수정'}
        footer={
          <>
        ...
        <div className="space-y-4">
          <div>
            ...
            <input className="input" value={form.code} onChange={e => setForm(f => ({ ...f, code: e.target.value }))}
              placeholder="예: PATIENT_DIRECT" disabled={modal === 'edit'} />
          </div>
          <div>
          ...
          <div className="flex items-center gap-2">
            <input type="checkbox" id="is_active_es" checked={form.is_active}
              onChange={e => setForm(f => ({ ...f, is_active: e.target.checked }))} className="w-4 h-4 rounded" />
            <label htmlFor="is_active_es" className="text-sm text-slate-700 cursor-pointer">사용여부</label>
          </div>
        </div>
      </Modal>
    </div>
