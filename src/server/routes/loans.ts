  ...
  try {
    ...
    const rows = await (prisma as any).$queryRawUnsafe(`
      SELECT dl.*, fi.name AS from_department_name, ti.name AS to_department_name,
             i.name AS item_name, i.item_code, i.uom, u.display_name AS creator_name
...
router.post('/', requirePermission('REQUEST_USE', 'SYSTEM_ADMIN'), async (req: AuthRequest, res) => {
  try {
    ...
    if (!all && userDept && userDept !== fromDepartmentId && userDept !== toDepartmentId) {
      return res.status(403).json({ error: '본인 관련 부서에만 등록할 수 있습니다.' });
    }
